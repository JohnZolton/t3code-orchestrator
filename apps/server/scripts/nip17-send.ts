...
if (!BOT_NSEC_HEX) { console.error("BOT_NSEC required"); process.exit(1); }
...
if (!recipientHex) { console.error("Usage: nip17-send.ts <npub/hex> [message]"); process.exit(1); }
...
const inboxEvents = await pool.querySync(discoveryRelays, { kinds: [10050, 10002], authors: [recipientHex], limit: 5 } as any);
...
const relays = inboxRelays.length > 0 ? [...new Set([...inboxRelays, ...discoveryRelays])] : discoveryRelays;
console.log(`Recipient inbox relays: ${inboxRelays.length > 0 ? inboxRelays.join(", ") : "(none found, using defaults)"}`);
console.log(`Publishing to: ${relays.join(", ")}`);

console.log(`Publishing to ${relays.length} relays...`);
const results = await Promise.allSettled(
  pool.publish(relays, giftWrap as any).map((p) =>
...
const timeouts = results.filter((r) => r.status === "rejected" || (r.status === "fulfilled" && r.value === "timeout")).length;
