...
async function createGiftWrappedDm(recipientPubkey: string, content: string): Promise<any> {
...
async function main() {
  ...
  try {
    await Promise.allSettled([
      ...pool
        .publish(DEFAULT_RELAYS, relayListEvent as any)
        .map((p) => Promise.race([p, new Promise((r) => setTimeout(r, 5000))])),
      ...pool
        .publish(DEFAULT_RELAYS, inboxRelayEvent as any)
        .map((p) => Promise.race([p, new Promise((r) => setTimeout(r, 5000))])),
    ]);
  ...
  pool.subscribeMany(DEFAULT_RELAYS, { kinds: [1059], "#p": [botPubkeyHex] } as any, {
    onevent: async (event: any) => {
      ...
      try {
        ...
        try {
          const inboxEvents = await pool.querySync(DEFAULT_RELAYS, {
            kinds: [10050, 10002],
            authors: [rumor.pubkey],
            limit: 3,
          } as any);
        ...
        await Promise.allSettled(
          pool
            .publish(replyRelays, giftWrap as any)
            .map((p) => Promise.race([p, new Promise((r) => setTimeout(r, 5000))])),
        );
