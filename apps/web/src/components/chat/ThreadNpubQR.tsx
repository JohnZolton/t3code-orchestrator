...
export const ThreadNpubQR = memo(function ThreadNpubQR({
  environmentId,
  threadId,
}: ThreadNpubQRProps) {
  ...
  const handleSendDm = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to send thread DM:", e);
    } finally {
      setSending(false);
    }
    ...
    <div className="relative flex items-center gap-1">
      <Tooltip>
        <TooltipTrigger
          render={
            <Toggle
              className="shrink-0"
              pressed={sent}
              onPressedChange={handleSendDm}
              aria-label="Send thread to phone via Nostr DM"
              variant="outline"
              size="xs"
              disabled={sending}
            >
        ...
        <TooltipPopup side="bottom">
          {sending ? "Sending..." : sent ? "Sent to phone!" : "Send thread to phone"}
        </TooltipPopup>
      </Tooltip>
      <Tooltip>
        <TooltipTrigger
          render={
            <Toggle
              className="shrink-0"
              pressed={open}
              onPressedChange={handleToggleQR}
              aria-label="Show thread QR code"
              variant="outline"
              size="xs"
              disabled={loading}
            >
